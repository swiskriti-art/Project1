...
## Get mid time for the JWAP, JWAP time range
...
```{r}
# Get data for a year from the PUMS API based on the variables given
# num_vars: Numerical variable to be return 
# cat_vars: categorical variable to be return
# geo: geography
# filter: list of numeric var filter, categorical var filter, and geography filter
get_data <- function(year=2022, num_vars= c("PWGTP", "AGEP"),
  ...
  tbl <- tbl |>
        mutate(across(setdiff(num_vars, c("JWAP", "JWDP")), as.numeric),
               across(starts_with("JW") & ends_with("P"), 
                      ~ifelse(cur_column() == "JWAP", jwap_map[.], jwdp_map[.])
                      ))
  return(tbl)
}
```
...
summaryfx <- function(tib, numvars = c("AGEP","GASP","GRPIP","JWMNP"), charvars = c("FER","HHL","HISPEED","SCH","SCHL","SEX")){
