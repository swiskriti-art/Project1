...
```{r}
mid_time <- Vectorize(function(colname, time_str) {
  map_tm <- switch(colname,
                   "JWAP"= jwap_map,
                   "JWDP" = jwdp_map)
  if(is.na(map_tm[time_str])) {
    return(NA)
  }
  parts <- str_split(map_tm[time_str], " to ")
  
  mid <- hm(parts[[1]][1]) + ((hm(parts[[1]][2]) - hm(parts[[1]][1]))/2)
  ampm = ifelse(str_detect(time_str, "am", "pm"))
  tm <- paste0(mid$hour,":",mid$minute," ",ampm)
  return(tm)
})  
```
...
```
# Getting the data
## JWTRNS excluded due to error message
```{r}
dt_tbl <- get_data(num_vars = c("PWGTP", "AGEP", "GASP", "GRPIP","JWAP", "JWDP", "JWMNP"), cat_vars = c("FER", "HHL", "HISPEED", "SCH", "SCHL", "SEX"),
                   geo = "State", geo_filter = "10")
dt_tbl
```
...
tib$AGEP <- as.numeric(tib$AGEP)
tib$GASP <- as.numeric(tib$GASP)
tib$GRPIP <- as.numeric(tib$GRPIP)
tib$JWAP <- as.numeric(tib$JWAP)
tib$JWDP <- as.numeric(tib$JWDP)
tib$JWMNP <- as.numeric(tib$JWMNP)

#hard-coded weighted columns -- there is opportunity to loop here 
tib$PWGTP_wgt <- tib$PWGTP * tib$PWGTP
tib$AGEP_wgt <- tib$AGEP * tib$PWGTP
tib$GASP_wgt <- tib$GASP * tib$PWGTP
tib$GRPIP_wgt <- tib$GRPIP * tib$PWGTP
tib$JWAP_wgt <- tib$JWAP * tib$PWGTP
tib$JWDP_wgt <- tib$JWDP * tib$PWGTP
tib$JWMNP_wgt <- tib$JWMNP * tib$PWGTP

tib$PWGTP_sqwgt <- (tib$PWGTP^2) * tib$PWGTP
tib$AGEP_sqwgt <- (tib$AGEP^2) * tib$PWGTP
tib$GASP_sqwgt <- (tib$GASP^2) * tib$PWGTP
tib$GRPIP_sqwgt <- (tib$GRPIP^2) * tib$PWGTP
tib$JWAP_sqwgt <- (tib$JWAP^2) * tib$PWGTP
tib$JWDP_sqwgt <- (tib$JWDP^2) * tib$PWGTP
tib$JWMNP_sqwgt <- (tib$JWMNP^2) * tib$PWGTP


summaryfx <- function(tib, numvars = c("AGEP","GASP","GRPIP","JWAP","JWDP","JWMNP"), charvars = c("FER_fac","HHL_fac","HISPEED_fac","SCH_fac","SCHL_fac","SEX_fac")){
  ...
  if(!("census" %in% class(tib))){
    ...
    #weighted standard deviation formula per instructions
      #Standard deviations are > than means... could this be due to weighting?
    dnm = sum(tib$PWGTP) - (wgtavg^2)
    num = sapply(tib[,paste0(numvars,"_sqwgt")],sum)
    stddev = sqrt(num/dnm)
    
    names(wgtavg) <- str_replace(paste0(names(wgtavg)," Mean"),"_wgt","")
    names(stddev) <- str_replace(paste0(names(stddev)," StdDev"),"_sqwgt","")
    numlist <- list(wgtavg, stddev)
...
summaryfx(tib)
summaryfx(tib, numvars = c("AGEP","GRPIP"), charvars = "SCH_fac")
...
```{r}
plotfx <- function(tib, numvars = "AGEP", charvars = "SEX_fac"){
...
plotfx(tib,numvars = c("AGEP"),charvars = "HISPEED_fac")
